...
function createHeader(headerValues, instanceURL, requestToken, conversation) {
  ...
  // Assign Quantity dynamically
  requisitionPayload.lines[0].Quantity = headerValues.quantity;
  requisitionPayload.lines[0].distributions[0].Quantity = headerValues.quantity;
...
exports.processRequisition = async function (payload, callback, conversation, done) {
  try {
    ...
    headerValues.quantity = payload.quantity;
